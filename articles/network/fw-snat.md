...
> [!NOTE] この記事は当初 Azure Firewall のパブリック IP の使われ方の説明がドキュメントになかったため、実際のパブリック IP の使われ方を説明しておりました。しかしながら、現在はドキュメントにパブリック IP の使われ方の説明が記載されているため、内容を Azure Firewall のパブリック IP と NAT ゲートウェイ併用構成の比較に変更いたしました。
...
注意点として、NAT ゲートウェイ V1 はゾーン冗長をサポートしていないため、ゾーン冗長を利用する場合は NAT ゲートウェイ V2 をデプロイする必要があります。
...
| パブリック IP が使用される順番           |  1 つ目がランダムに選択され、ポートが枯渇すると別の IP が使用される  |    ランダムかつほぼ均等に使用される    |
| ゾーン冗長のサポート              |  ○  |     ○ (V2 でサポート)    |
| 価格              |  FW と FW のパブリック IP の価格  |  FW と FW のパブリック IP, NAT ゲートウェイと NAT ゲートウェイ のパブリック IP の価格    |

※1 Azure Firewall の内部インスタンス数は既定で最低 2 となり、負荷に応じてスケールアウトします。また、手動でのスケーリングも行えます。
