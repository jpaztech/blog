...
Azure では、インターネット宛てのアウトバウンド接続を何種類かの方法で構成することができます。選択の幅があることは良いことですが、一方で各構成の内容を把握するのが厄介なのも事実です。そこで本記事では、Azure VM に対して使える送信接続構成の一覧、どの構成によって Azure VM が接続しているかの見分け方、そして各構成の特徴をご紹介します。
...
> - VM の仮想 NIC にパブリック IP アドレスを関連付けている
...
> - VM に仮想 NIC にパブリック IP アドレスを関連付ける ([参考](https://learn.microsoft.com/ja-jp/azure/virtual-network/ip-services/associate-public-ip-address-vm?tabs=azure-portal))
...
Azure VM をはじめとする仮想ネットワーク (VNet) のコンピューティング リソースは、必要に応じてインターネット上のホストに対して接続を行います。たとえば、Web ブラウザが TLS/SSL 証明書の失効を検証する場合や、DNS サーバーが外部の権威 DNS サーバーに再帰問い合わせを実施する場合などです。この接続は、Azure から外部に出ていくように見えることから、アウトバウンド方向の接続あるいは **送信接続** と呼ばれます。
...
1. 送信元のホスト (オペレーティング システムが管理するネットワーク インターフェイス) に、パブリック IP アドレスを割り当てます。これで送信元ホストはインターネットのセグメントに参加する状態となり、宛先ホストとエンド・ツー・エンドな通信が行えます。
2. 送信元のホストには、プライベート IP アドレスのみを割り当てます。その代わり、送信元ホストの外部にあるネットワーク装置で、送信元 IP アドレスをパブリック IP アドレスに変換します。

このうち、Azure でサポートされているのは 2 番目の方法です。これは、ネットワークのアドレス変換は Network Address Translation (NAT) と呼ばれる処理で、送信元を変換する場合は Source NAT (SNAT) と呼ばれることもあります。後述するネットワーク構成パターンを取ることで、Azure の基盤に実装された仮想ルーターが Azure VM のパケットに対して SNAT を実施します。

また、NAT (SNAT) はさらに 2 つの方式に細分化できます。

- **Basic NAT**: 二つの IP アドレスを 1 対 1 の関係で変換する NAT です。狭義の NAT と呼ばれることもあります。NAT 装置上では、単純にアドレスの対応付けを覚えておくだけで良いので、ステート管理が必要ありません。
- **NAPT (Network Address Port Translation)**: ポート番号を利用することで、二つのアドレスを 1 対多あるいは多対多の関係で変換できる NAT です。アドレスの対応付け情報だけでは実現できず、5 タプル (送信元アドレス、宛先アドレス、プロトコル、送信元ポート、宛先ポート) で一意にフローを識別しながら、どのフローをどのように NAT したか管理しておく必要があります。このフロー管理のデータベースを NAT テーブルと呼びます。

特に、後者の NAPT による NAT (SNAT) では、**SNAT ポート枯渇**に気を付けてください。

SNAT ポートとは、SNAT 後のパケットの送信元ポート番号です。1 ~ 65535 の値が利用できますが、重要なのは有限である点です。基本的に、[プロトコル、宛先 IP、宛先ポート] が同一のコネクションが発生するたびに SNAT ポートを一つづつ消費するため、場合によってはすべての SNAT ポート消費しきってしまう可能性があるのです。特に、ホストごとに使える SNAT ポートをあらかじめアサインしておく構成において、ポート枯渇が問題となることが多いです。

![snat-port-exhaustion](./snat-options-for-azure-vm/snat-port-exhaustion.png)

SNAT ポートがいかに消費されるかを示すため、たとえば次のような状況を考えます。あるSNAT 装置は、単一の SNAT 用パブリック IP アドレス `SNAT` を保有しています。そして `10.0.0.4` と `10.0.0.5` のプライベート IP アドレスをもつ Azure VM #1 と #2 に対して NAPT による SNAT を提供し、各 VM には 100 個ずつ SNAT ポートを事前に割り当てることにします。

この時、VM からある Web サーバーに対する HTTP アクセスを大量に発生させました。この通信は、[プロトコル、宛先 IP アドレス、宛先ポート] がすべて同一となる点に留意してください。戻りのパケットがどのコネクションであるか識別しなければならない SNAT 装置は、SNAT ポートをコネクションごとに新しく割り振りますが、VM#1 からの 101 個目の接続を SNAT する時、新たな VM#1 向けの SNAT ポートが存在しないことに気が付きます。これがまさに SNAT ポートが枯渇した状態です。

一度使ったSNAT ポートを再利用するには、既存のコネクションを FIN/RST で終了するか、アイドルタイムアウトで強制的に切断されるのを待つ必要があります。また、異なるプロトコルや宛先アドレス、ポート番号のフローに対しては、また別で SNAT ポートが管理されるため、ポート枯渇の問題は影響しません。

ポート枯渇が発生しやすいのは、同じエンドポイントに対して似たような送信接続を繰り返す場面です。たとえば次のようなシナリオです。
...
3. 仮想 NIC にパブリック IP アドレス (インスタンス レベルのパブリック IP アドレス) を関連付ける
...
ここでは、特定の Azure VM がどの構成パターンで送信接続を実現しているか判定するためのフローチャートを示します。

より厳密に言えば、これは仮想 NIC に対しての判定チャートです。なぜなら、同一 Azure VM にアタッチする仮想 NIC であっても、仮想 NIC ごとに異なる送信接続が構成できるためです。たとえば、異なる二つのサブネットに足を出している Azure VM に対し、片方のサブネットでは NAT ゲートウェイを使った送信接続を、もう一方のサブネットでは Azure Firewall を使った送信接続を構成することが可能です。

またこのフローチャートは、ある仮想 NIC に複数の構成パターンを構成している場合の優先度も考慮して作成されています。たとえば、NAT ゲートウェイとインスタンス レベルのパブリック IP アドレスが両方構成された場合、NAT ゲートウェイによる SNAT で送信接続が行われます。
...
本ステップは、仮想アプライアンスでインターネット トラフィックを処理する構成に合致しているかを確認するものです。

要件によっては、インターネット通信が必ず通るようなネットワーク仮想アプライアンス (NVA) をデプロイし、ログの記録やトラフィック制御 (e.g. ブロック、ヘッダの書き換え) を実施することがあります。マネージドな仮想アプライアンスとして Mcirosoft から Azure Firewall が提供されていますが、他にも多くのサードパーティ製の製品が存在します。こうした構成を取る場合、Azure VM からインターネット宛てのエグレス トラフィックの SNAT は、VM 側ではなく NVA 側で実施されます。

具体的には、Azure VM の[有効なルート](https://learn.microsoft.com/ja-jp/azure/virtual-network/diagnose-network-routing-problem#diagnose-using-azure-portal)を見て、ネクストホップが NVA に向けられているか確認します。**既定のデフォルトルート (0.0.0.0/0) を無効化するユーザー定義ルートがあるか**を重点的に確認していただくのが良いでしょう。

たとえば、次のような状況の場合について考えてみます。

![effective-routes-afw](./snat-options-for-azure-vm/effective-routes-afw.png)

まず、インターネット トラフィック全般を表すデフォルト ルート (0.0.0.0/0) が、仮想アプライアンス (10.0.255.4) に向いている状態であることが確認できます。ただし、[KMS サーバーをホストする 3 つのアドレス](https://jpaztech.github.io/blog/vm/azure-kms-update/)に関しては、ネクストホップがインターネットに設定されています。これらに関しては、アプライアンスを使わず VM 側で SNAT することが意図されています。したがって、この状況での当該ステップへの回答は次の通りです。
...
本ステップでは、Azure VM の仮想 NIC がデプロイされているサブネットのプロパティを確認します。様々な方法で確認できますが、Azure Portal での確認方法の一例を以下に示します。

1. 対象の Azure VM の [ネットワーク] メニューで、仮想 NIC がデプロイされている仮想ネットワーク/サブネットを確認する
...
#### 仮想 NIC にパブリック IP アドレスが関連付けられている？

本ステップでは、Azure VM の仮想 NIC にパブリック IP アドレスが関連付けられているか確認します。このようなパブリック IP アドレスはインスタンス レベル IP アドレスとも呼ばれます。[パブリック IP アドレスの関連付けの手順](https://learn.microsoft.com/ja-jp/azure/virtual-network/ip-services/associate-public-ip-address-vm?tabs=azure-portal)を参考に、次のように確認してください。

1. 対象の仮想 NIC の [IP 構成] メニューで、プライマリの IP 構成を確認する
...
1. 対象の Azure VM の [ネットワーク] メニューで、対象の仮想 NIC を確認します
...
既定の送信アクセスとは、特に何も構成していなくても最低限の SNAT 機能を担保することを目的とした、Azure 基盤の SNAT 機能です。以下の条件を満たせば、仮想 NIC は既定の送信アクセスにより SNAT できます。

**条件その１**: 同じ可用性セットに含まれるすべての仮想 NIC が、次の状態に合致しない。

- Standard SKU のインスタンス レベルのパブリック IP アドレスを仮想 NIC に関連付ける
- Standard SKU のロードバランサーのバックエンドプールに仮想 NIC を配置する

可用性セットを作成していない場合は、便宜上、単一の Azure VM からなる可用性セットを考えるか、あるいは単に可用性セットを Azure VM と読み替えていただいて大丈夫です。同様に、VMSS のインスタンスの場合は、可用性セットを VMSS と読み替えてください。


> [!NOTE]
> なぜこのような条件になっているかというと、Azure 基盤が持つリソース保護機能が関係しています。
>
> Azuer VM が Standard SKU のパブリック IP アドレスやロードバランサーを使用すると、内部的にセキュリティが強化された保護モードに切り替わります。保護のスコープは可用性セットであり、その中のすべての仮想 NIC に同じ保護モードが適用されます。なお、可用性セットが組まれていなければ、単一の Azure VM で構成された可用性セットが内部的に作成されます (ユーザーには見えない)。
>
> 当該の保護モードの結果、明示的に NSG のセキュリティ規則で通信許可をするまではインターネットの通信ができないようになります。そして、この保護モードのもう一つの特徴が、既定の送信アクセスを無効にするという点です。
> 
> したがって、Azure VM が既定の送信アクセスの対象となるためには、保護モードの対象になっていないことを確認する必要があるのです。

さらにややこしいことに、実は既定の送信アクセスを "明示的に" 構成することも可能です。細かいエッジケースなので普段は気にする必要はないかもしれませんが、念のため掲載しています。具体的には、以下の条件を満たす場合でも既定の送信アクセスが適用されます (条件その１と２は同時に満たす必要はありません)。

**条件その２**: Standard SKU のパブリック ロードバランサーに「既定の送信アクセス」を有効化した負荷分散規則が存在し、その規則のバックエンド プールに仮想 NIC が存在する。
...
* Standard SKU のパブリック IP アドレスを Azure VM の仮想 NIC に関連付ける
...
|:----|:----|:----|:----|:----|:----|
| 構成単位 | サブネット | サブネット | 仮想 NIC | バックエンド プール | VM / 可用性セット |
...
- **構成単位**: 同じ送信接続が構成されるスコープです。たとえば、サブネットが構成単位である場合、サブネット内のすべての仮想 NIC は同じ方法で送信接続されます。
...
- **最大 SNAT ポート数**: 仮想 NIC が利用できる最大の SNAT ポート数を表します。NAT ゲートウェイでは、利用状況に応じて SNAT ポートを動的に分配するため、他の Azure VM の稼働状況にも左右されます。理論上、NAT ゲートウェイ全体で最大 64,512 * 16 = 1,032,192 個の SNAT ポートが確保できます。
...
- NAT ゲートウェイを使用する仮想 NIC は、Basic SKU のインスタンス レベル パブリック IP アドレスを関連付けたり、Basic SKU のロードバランサーのバックエンドに配置できません。
...
Azure VM の仮想 NIC にパブリック IP アドレス (インスタンス レベルのパブリック IP アドレス) を関連付けると、そのパブリック IP アドレスで通信が SNAT されるようになります。1 対 1 のステートレス NAT (Basic NAT) として実装されるため、基盤側では SNAT ポートを管理しません。
...
2. バックエンド プールに対象の Azure VM の仮想 NIC を追加する。
3. 送信規則を作成する。
...
> [!NOTE]
> 例外的に、パブリック ロードバランサーの負荷分散規則を使用することでも、既定の送信アクセスを実現することができます。この場合、SNAT のアドレスはロードバランサーのフロントエンド IP 構成に紐づくパブリック IP アドレスとなります。条件の詳細は、判定フローチャートの「既定の送信アクセスに該当する？」をご覧ください。

**特徴**

* 唯一パブリック IP アドレス リソースを作らずに SNAT が出来る送信接続の構成です。
* NAPT により実現されており、確保される SNAT ポート数は仮想 NIC (Azure VM) あたり 1024 で固定されています。
...
「判定フローチャート」を使うと、対象の Azure VM (仮想 NIC) がどの方式の送信接続で構成されているのか簡単に調べられます。送信接続できない時のトラブルシューティングにも、ぜひこのフローチャートを活用していただければと思います。
