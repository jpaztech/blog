...
> [!NOTE]
> Azure Files については、プロビジョニング済み v2 の課金モデルが追加されました。  
> プロビジョニング済み v2 の課金モデルの各サポート状況については、公式ドキュメントをご参照いただけますと幸いです。  
> ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝  
> ■ご参考：Azure Files のデータ冗長性  
> https://learn.microsoft.com/ja-jp/azure/storage/files/files-redundancy  
> ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝  
> 本 Blog では Azure Files について、課金モデルがプロビジョニング済み v1（Premium ファイル共有）と従量課金制（Standard 汎用 v2 ストレージアカウント）に関しての記載となります点、ご了承ください。  
