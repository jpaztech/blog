...
> [!TIP]
> 以前は WindowsAzureTelemetryService というサービスも稼働しておりましたが、2020 年頃のバージョン 2.7.41491.971 から、WindowsAzureTelemetryService は Windows Azure Guest Agent サービスに統合されております。
> もしご利用環境に当該サービスが存在する場合は後述のサービス停止、削除手順の引数に WindowsAzureTelemetryService を指定して追加でご実行ください。
