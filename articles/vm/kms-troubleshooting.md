...
Azure 上の Windows VM は定期的に KMS ライセンス認証をする必要がございます。  
そのため、KMS のライセンス認証が失敗し続けた場合ライセンス認証切れに関する警告が表示されることがございます。  
本件について多くのお問い合わせを頂きますため、KMS のライセンス認証のトラブルシューティング等について以下の内容をご紹介させていただきます。

- KMS ライセンス認証についての解説
  - KMS ライセンス認証の概要
  - KMS ライセンス認証に必要な設定および通信要件
  - 自動的なライセンス認証および期間について
  - ライセンス認証切れの影響
...
先述の通り、Azure 上の Windows VM は定期的に KMS ライセンス認証をする必要がございます。  
...
ゲスト OS となる Windows からは KMS（Key Management Services）サーバーに、定期的なライセンス認証を行う必要がございます。  
このライセンス認証先の KMS サーバーは Azure 上にマイクロソフトがご用意しております。  
そのため、お客様のご利用の Windows VM から KMS サーバーへのライセンス認証の通信経路が確保されている必要がございます。

> [!TIP]
> Azure 上にお客様ご自身で KMS ホストサーバーの役割を担うために Windows Server 構築いただく場合は、例外的に通常の Azure Windows VM の KMS ライセンス認証ではなく、オンプレミス環境と同じような KMS ホストサーバー用のライセンスキーを使用したライセンス認証が必要となります。
...
> [!TIP]
> KMS ライセンスサーバーへのライセンス認証通信については、NSG（ネットワークセキュリティグループ）の影響を受けないようになっております。  
> すなわち、NSG での明示的な通信許可は不要となります。  
...
また、ライセンス認証は毎回必ず成功する必要はなく、以下の期間の間で成功することで、都度有効期限が延長されます。

- ライセンス認証が 1 度も成功していない状態：30 日間の猶予期間となっている
- ライセンス認証が成功した場合：180 日の有効期限が設定される
...
この際の影響は以下のようなものとなり VM が使用不可になるといった影響はございませんが、ライセンスは認証状態となる前にライセンス認証をお願いいたします。

1. 画面の右下にライセンス認証が必要であることを示すウォーターマーク (透かし文字) が表示されます。
2. 壁紙の右下に OS の情報を示す文字が表示されます。
3. パーソナル設定内の項目は設定変更不可となります。
4. KMS ホストの場合、KMS クライアントの認証が行えません。
5. Windows Update はセキュリティ更新プログラムと重要な更新プログラムだけをインストールし、オプションの更新プログラムはインストールしません。

■ご参考：Windows ライセンス認証が期限切れになるとどうなりますか?
https://docs.microsoft.com/ja-jp/azure/virtual-machines/troubleshooting/troubleshoot-activation-problems#what-happens-if-windows-activation-period-expires
...
![](./kms-troubleshooting/2023-08-02-11-38-39.png)

また、ライセンス認証が切れた際にメッセージが表示されるものと存じます。

![](./kms-troubleshooting/2023-08-02-11-39-40.png)
...
そのため、まずは先述の必要な通信要件等が可能な状況が確認いただくことが必要です。  
正常なライセンス認証を行うためのトラブルシューティング方法を解説させていただきます。  
...
別途修正等を行った後にライセンス認証が成功するか確認する際にもこちらのコマンドをご利用いただけます。  
管理者特権の Windows PowerShell プロンプトにて実行をお願いいたします。
...
ライセンス認証の成功・失敗については Event Viewer を用いて、  
...
後述の「KMS ホストサーバーの一時的な負荷」のセクションに記載の通り、一時的な KMS ホストサーバーの負荷の問題で表示される可能性がございますが、「12289：ライセンス認証の結果表示」にて正常にライセンス認証がされている場合は無視可能となります。  
...
設定方法については、[「強制トンネリング環境（直接通信となっていない）」](https://jpaztech.github.io/blog/vm/kms-troubleshooting/#%E5%BC%B7%E5%88%B6%E3%83%88%E3%83%B3%E3%83%8D%E3%83%AA%E3%83%B3%E3%82%B0%E7%92%B0%E5%A2%83%EF%BC%88%E7%9B%B4%E6%8E%A5%E9%80%9A%E4%BF%A1%E3%81%A8%E3%81%AA%E3%81%A3%E3%81%A6%E3%81%84%E3%81%AA%E3%81%84%EF%BC%89) セクションの解決方法と同じになりますので、そちらをご参照ください。  
...
| IP アドレス | 20.118.99.22, 40.83.235.53, 23.102.135.246 |
| ポート | 1688 |
| プロトコル | TCP |
...
すなわち、KMS サーバー（20.118.99.224, 40.83.235.53, 23.102.135.246）への通信のみ、直接通信とするために Next Pop を Internet にします。
...
左側のメニューより「ルート」を選択し「追加」より、KMS サーバーへの通信のみ、直接通信とするために Next Pop を Internet とするルートを 3 つそれぞれ作成します。  
...
ライセンス認証を行ったところ以下のように「エラー コード: 0xC004E015」が表示される場合がございます。  

![](./kms-troubleshooting/2023-08-07-09-13-40.png)
